/**
 * rbp-shared.js — RugbyBoard Pro v2.5
 * Fullscreen mobile drawer · Visible lang buttons · Perfect responsive · Full i18n
 */
(function () {
  'use strict';
...
#rbpNav {
  ...
  transition: box-shadow .3s; font-family: 'Montserrat', sans-serif;
  box-sizing: border-box;
}
#rbpNav.rbp-scrolled { box-shadow: 0 4px 40px rgba(0,0,0,.9); }
...
.rbp-lang-btn {
  ...
  border:1.5px solid rgba(200,255,0,.38); background:rgba(200,255,0,.1);
  color:rgba(200,255,0,.8); cursor:pointer;
  display:inline-flex; align-items:center; justify-content:center;
  font-family:'Montserrat',sans-serif; font-weight:800; font-size:.64rem; letter-spacing:.06em;
  transition:all .18s; white-space:nowrap;
}
.rbp-lang-btn:hover { border-color:#c8ff00; background:rgba(200,255,0,.2); color:#c8ff00; transform:scale(1.05); }
...
.rbp-burger-btn {
  ...
  border:1.5px solid rgba(200,255,0,.32); background:rgba(200,255,0,.08);
...
.rbp-menu-overlay {
  ...
  background:rgba(1,4,9,.97); backdrop-filter:blur(16px); -webkit-backdrop-filter:blur(16px);
  flex-direction:column; align-items:center; justify-content:center;
  font-family:'Montserrat',sans-serif;
  opacity:0; transition:opacity .3s ease;
}
.rbp-menu-overlay.rbp-is-open {
...
.rbp-overlay-close:hover { background:rgba(200,255,0,.2); border-color:#c8ff00; }

/* Logo top-left */
.rbp-overlay-logo {
  position:absolute; top:1.1rem; left:1.25rem;
}
.rbp-overlay-logo img { height:38px; width:auto; }

/* Nav links — big and centered */
.rbp-overlay-links {
...
.rbp-mobile-link {
  ...
  color:rgba(240,246,252,.75); text-decoration:none;
  font-weight:800; font-size:1.1rem; letter-spacing:.06em; text-transform:uppercase;
  transition:all .2s; text-align:center;
}
.rbp-mobile-link:hover, .rbp-mobile-link.rbp-active {
  color:#c8ff00; border-color:rgba(200,255,0,.2); background:rgba(200,255,0,.06);
}
...
.rbp-mobile-cta {
  ...
  box-shadow:0 0 30px rgba(200,255,0,.4); margin-bottom:2.5rem;
  text-decoration:none;
}
.rbp-mobile-cta:hover { opacity:.88; transform:translateY(-2px); box-shadow:0 0 50px rgba(200,255,0,.6); }

/* Lang in overlay */
.rbp-overlay-lang { display:flex; gap:.5rem; align-items:center; }
.rbp-overlay-lang .rbp-lang-btn {
  height:36px; padding:0 .9rem; font-size:.72rem;
}

/* ── FOOTER ── */
#rbpFooter {
  border-top:1px solid rgba(255,255,255,.06);
  padding:4rem 2.5rem 2.5rem; font-family:'Montserrat',sans-serif;
}
...
@media (max-width:440px) {
  #rbpNav { height:56px; padding:0 1rem; }
  .rbp-logo img { height:36px; }
  .rbp-burger-btn { width:34px; height:34px; }
}
...
/* Prevent body overflow from clipping fixed elements */
.landing-wrapper, .content-wrapper { overflow:visible !important; }
`;
  ...
  const T = {
    ...
    en: {
      ...
      featuresTitle:'Pro Tools for Coaches', featuresSubtitle:'Everything you need to build winning strategies and communicate them effectively',
      ...
      f6Title:'Real-Time Collaboration', f6Desc:'Share plays with your coaching staff. Work simultaneously and get instant feedback.',
    ...
    it: {
      ...
      featuresTitle:'Strumenti Pro per Allenatori', featuresSubtitle:'Tutto ciò che ti serve per creare strategie vincenti e comunicarle efficacemente',
      ...
      f4Title:'Libreria di Schemi', f4Desc:'Archivia e organizza tutte le strategie. Cerca, filtra e riutilizza gli schemi salvati.',
      f5Title:'Personalizzazione Totale', f5Desc:'Personalizza campi, giocatori, colori della squadra e stili secondo il tuo brand.',
  ...
  function langBtns(cls='') {
    return LANGS.map(l =>
      `<button class="rbp-lang-btn${cls}${currentLang===l.code?' rbp-is-active':''}" data-rbp-lang="${l.code}" type="button">${l.label}</button>`
    ).join('');
  }
  ...
  function buildNav(d) {
    ...
    // Fullscreen overlay links — big, centered
    const overlayLinks = links.map(l =>
      `<a href="${l.href}" class="rbp-mobile-link${isActive(l.href)?' rbp-active':''}" data-rbp-i18n="${l.key}">${d[l.key]}</a>`
    ).join('');
...
<div class="rbp-menu-overlay" id="rbpOverlay" role="dialog" aria-modal="true" aria-label="Menú de navegación">
  <a class="rbp-overlay-logo" href="index.html" aria-label="RugbyBoard Pro">
    <img src="rbp_Italic.png" alt="RugbyBoard Pro" width="120" height="38"/>
  </a>
  <button class="rbp-overlay-close" id="rbpOverlayClose" type="button" aria-label="Cerrar menú">✕</button>

  <div class="rbp-overlay-links">
    ${overlayLinks}
  </div>
...
<footer id="rbpFooter" role="contentinfo">
  ...
  function applyLang(lang) {
    ...
    document.querySelectorAll('[data-rbp-i18n]').forEach(el => {
      const v = d[el.dataset.rbpI18n]; if (v !== undefined) el.textContent = v;
    });
  ...
  function bindEvents() {
    ...
    const closeBtn   = document.getElementById('rbpOverlayClose');
    ...
    const open = () => {
      ...
      setTimeout(() => overlay.querySelector('.rbp-mobile-link')?.focus(), 80);
    };
    const close = () => {
    ...
    burger.addEventListener('click', () => overlay.classList.contains('rbp-is-open') ? close() : open());
    closeBtn?.addEventListener('click', close);

    // Close on link click
    overlay.querySelectorAll('.rbp-mobile-link, .rbp-mobile-cta').forEach(el =>
